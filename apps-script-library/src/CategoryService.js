...
class CategoryService {
  ...
  initializeSettingsSheet() {
    ...
    this.settingsSheet = ss.getSheetByName(SHEET_NAMES.GENERAL_SETTINGS);
    
    if (!this.settingsSheet) {
      throw new Error(`"${SHEET_NAMES.GENERAL_SETTINGS}" sheet not found. Please create a Settings sheet with categories.`);
    }
  ...
  loadCategories() {
    try {
      ...
      if (lastRow < 3) {
        throw new Error(`No categories found in "${SHEET_NAMES.GENERAL_SETTINGS}" sheet. Please add categories starting from row 3.`);
      }
      ...
      if (Object.keys(this.categories).length === 0) {
        throw new Error(`No valid categories found in "${SHEET_NAMES.GENERAL_SETTINGS}" sheet.`);
      }

      Logger.log(`Loaded ${Object.keys(this.categories).length} categories from "${SHEET_NAMES.GENERAL_SETTINGS}" sheet:`, Object.keys(this.categories));
  ...
  refreshCategories() {
    this.loadCategories();
    Logger.log(`Categories refreshed from "${SHEET_NAMES.GENERAL_SETTINGS}" sheet`);
  }
